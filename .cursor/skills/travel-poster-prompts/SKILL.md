...
| 数据源 | 写入提示词的用途 |
|--------|------------------|
| `content` | 城市/国家主题；`title` 用 **城市名称 + 主题 + 第x卷**（如「曼谷 · 热带街巷 · 第一卷」），勿用《…边记：…》式副标题 |
...
1. **语种**：`title`、`surrounding_panels`、`调色板` 以**简体中文**为主；必要时景点/店名保留原文。`title` 固定为 **城市名 + 主题 + 第x卷**（分隔可用「 · 」或短横线），不用「边记」等套话。
