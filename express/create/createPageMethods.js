...
function replaceTitle(dom) {
    ...
    for (let i = 0, len = names.length; i < len; i++) {
        ...
        let titleSymbol = new Array(i+1).fill(0).map(item => '#').join('')
...
module.exports = function (app) {
    app.post('/create/page', bodyParser.json(), (req, res) => {
        ...
        async function createMd() {
            createFile(`./docs${form.link}/${fileName}`, render(require(`./PAGE_TEMPLATE.js`), {
                ...
                password: form.password && form.passwordCus ? form.passwordCus : form.password,
                content: ''
            }))
        }
        ...
        var c = new Crawler({
            ...
            callback: function (error, _res, done) {
                ...
                if (error) {
                    ...
                    const title = dom.window.document.querySelector('.article-title').textContent.trim()
