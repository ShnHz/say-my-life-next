<template>
  <div class="weight-loss-map">
    <div class="bg">
      <svg viewBox="2 120.9 600 600">
        <defs>
          <path
            id="SVGID_1_"
...
<script setup lang="ts">
  ...
  onMounted(async () => {
    ...
    const fat = document.querySelector('#obese-belly')
    const bellyButton = document.querySelector('#obese-belly-button')
    const obeseLeft = document.querySelector('#obese-hand-left')
    const obeseRight = document.querySelector('#obese-hand-right')
    const shirt = document.querySelector('#obese-shirt')

    const gsap = (window as any).gsap

    gsap.set(fat, { transformOrigin: 'center' })
    ...
    gsap.set(obeseLeft, { transformOrigin: '11 37' })
    gsap.set(obeseRight, { transformOrigin: '135 56' })
    ...
    // 简化 shirt 动画，不使用 morphSVG（需要单独加载 MorphSVG 插件）
    gsap.fromTo(
      shirt,
      {
        scale: 1,
